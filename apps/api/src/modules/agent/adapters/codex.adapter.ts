...
export class CodexAdapter {
  ...
  getConfig(): AgentConfig {
    return {
      name: 'codex',
      modes: ['generate_pr', 'review_pr', 'repair_ci', 'summarize'],
      execution: ['cli', 'github_action', 'cloud'],
      capabilities: {
        readFiles: true,
        editFiles: true,
        runCommands: true,
  ...
  async prepare(task: AgentTaskContext, policy: EffectivePolicy): Promise<AgentRunPlan> {
    ...
    return {
      ...
      args: [
        ...
        'exec',
        '--skip-git-repo-check',
        '--sandbox', this.mapSandboxMode(policy.filesystem),
        '--json',
  ...
  async run(plan: AgentRunPlan): Promise<AsyncIterable<{ type: string; data: Record<string, unknown> }>> {
    return this.runtime.execute(plan, 'Codex execution started');
  }
  ...
  private mapSandboxMode(filesystem: EffectivePolicy['filesystem']): string {
    switch (filesystem) {
      case 'read_only':
        return 'read-only';
      case 'full_access':
